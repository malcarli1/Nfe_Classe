...
CLASS Malc_GeraXml
   ...
   // Grupos novos/avancados NT RTC/CNPJ alfa/PAA
   VAR aPagAntecipadoRef       AS Array     INIT {}
   VAR nVitemRtc               AS Num       INIT 0
   VAR nVitemRtc_t             AS Num       INIT 0
   VAR cDfeRefChave            AS Character INIT []
   VAR nDfeRefItem             AS Num       INIT 0
   VAR cAgropecuarioXml        AS Character INIT []
   VAR cInfPaaXml              AS Character INIT []
   ...
   METHOD AddPagAntecipadoRef()                                                   // NT RTC
   METHOD SetVItemRtc()                                                           // NT RTC
   METHOD SetDFeReferenciado()                                                    // NT RTC
   METHOD SetAgropecuarioXml()                                                    // NT RTC
   METHOD SetInfPaaXml()                                                          // PAA                                                       // Reforma tributária
   METHOD fCria_Autxml()
...
METHOD fCria_Ide()
   ::cXml+= "<ide>"                                                                                                              // Início da TAG (ide)
          ...
          Elseif ::cModelo == [55]
             ::cXml+= ::XmlTag( "tpImp" , Iif(!(::cTpimp $ [0_1_2_3_6]), [1], Left(::cTpimp, 1)))                                // Tipo de Impressão 0 - Sem geração de DANFE; 1 - DANFE normal, Retrato; 2 - DANFE normal, Paisagem; 3 - DANFE Simplificado; 6 - DANFE Simplificado Tipo 2
          EndIf 
          ...
          If Len(::aPagAntecipadoRef) > 0
             ::cXml+= "<gPagAntecipado>"
             AEval(::aPagAntecipadoRef, {| cRef | ::cXml+= ::XmlTag( "refNFe", Left(::SoNumeroCnpj(cRef), 44)) })
             ::cXml+= "</gPagAntecipado>"
          EndIf
   ::cXml+= "</ide>"
Return (Nil)
...
* -------------> Metodo para adicionar NF-e de pagamento antecipado <--------- *
METHOD AddPagAntecipadoRef(cChave)
   hb_Default(@cChave, [])
   If !Empty(cChave)
      AAdd(::aPagAntecipadoRef, cChave)
   EndIf
Return (Self)

* -------------> Metodo para informar vItem da Reforma Tributaria <----------- *
METHOD SetVItemRtc(nValor)
   hb_Default(@nValor, 0)
   ::nVitemRtc := nValor
Return (Self)

* -------------> Metodo para referenciar item de outro DFe <------------------ *
METHOD SetDFeReferenciado(cChaveAcesso, nItem)
   hb_Default(@cChaveAcesso, [])
   hb_Default(@nItem, 0)
   ::cDfeRefChave := cChaveAcesso
   ::nDfeRefItem  := nItem
Return (Self)

* -------------> Metodo para informar grupo agropecuario bruto <-------------- *
METHOD SetAgropecuarioXml(cXml)
   hb_Default(@cXml, [])
   ::cAgropecuarioXml := cXml
Return (Self)

* -------------> Metodo para informar grupo infPAA bruto <-------------------- *
METHOD SetInfPaaXml(cXml)
   hb_Default(@cXml, [])
   ::cInfPaaXml := cXml
Return (Self)
...
METHOD fCria_Produto()
   ::cXml+= [<det nItem="] + Left(NumberXml( ::nItem, 0 ), 3) + [">]
          ...
          If !Empty(::nVitemRtc)
             ::cXml+= ::XmlTag( "vItem", ::nVitemRtc)
             ::nVitemRtc_t+= ::nVitemRtc
          EndIf

          If !Empty(::cDfeRefChave)
             ::cXml+= "<DFeReferenciado>"
                    ::cXml+= ::XmlTag( "chaveAcesso", Left(::SoNumeroCnpj(::cDfeRefChave), 44))
                    If ::nDfeRefItem > 0
                       ::cXml+= ::XmlTag( "nItem", ::nDfeRefItem, 0)
                    EndIf
             ::cXml+= "</DFeReferenciado>"
          EndIf
   ::cXml+= "</det>"
...
METHOD fCria_ProdutoIs()                                                                                                         // Reforma tributária
   If !Empty(::cClasstribis)
      ::cXml+= "<IS>"
             ...
             ::cXml  += ::XmlTag( "vIS"          , ::nVis:= Round((::nVbcis * (::nPisis/100)) + (::nQtrib_is * (::nPisespec/100)), 2))                 // Valor do Imposto Seletivo
             ::nVis_t+= ::nVis
      ::cXml+= "</IS>"
   EndIf 
Return (Nil)
   ...
   If ::cModelo == [55]
      ...
      If !Empty(::cCredPrescbs) .and. ::cCredPrescbs $ [1_2_3_4_5]
         ::cXml+= "<gCBSCredPres>"
                ...
                ::cXml                  += ::XmlTag( "vCredPres" , ::nVcredprescbs:= Round(::nVbcibs * (::nPcredprescbs/100), 2) )
...
METHOD fCria_TotaisRtc()
   If !Empty(::cCclasstrib) .or. !Empty(::cClasstribis)
      ...
      If !Empty(::nVdifgibsuf_t)    .or. !Empty(::nVdevtribgibsuf_t)      .or. !Empty(::nVibsufgibsuf_t)   .or. ;                    // Tag só é gerada se houver valores informados nos itens, (Simples nacional não gera a Tag)
         ...
         ::cXml+= "</IBSCBSTot>"
      EndIf   
      If !Empty(::nVitemRtc_t)
         ::nvNftot:= ::nVitemRtc_t
         ::cXml+= ::XmlTag( "vNFTot" , ::nvNftot)  // Valor total da NF-e considerando IBS / CBS / IS
      EndIf   
      ::cXml+= "</total>"
   EndIf
Return (Nil)
...
METHOD fCria_Fechamento()
   If !Empty(::cAgropecuarioXml)
      ::cXml+= ::cAgropecuarioXml
   EndIf
...
METHOD fConsultaGTIN(cGtin)
   ...
   hb_MemoWrit(Malc_GetCurrentFolder() + [\retorno_gtin.xml], cRetorno)
...
METHOD ExtraiTagsRepetidas(cXml, cTag)
   ...
   Do While .T.
      nPosOpen:= Malc_At( cBuscaOpen, cXml, nIni )
      If nPosOpen == 0
         Exit
      Endif
      
      nPosOpen += Len(cBuscaOpen)
      nPosClose:= Malc_At( cBuscaClose, cXml, nPosOpen )
...
METHOD fConsultaCNPJ(cCnpj)
   Local oHttp, cRetorno, oErr, hResponse:= Malc_Hash(), nItem
   ...
   hb_MemoWrit(Malc_GetCurrentFolder() + [\retorno_cnpj.txt], cRetorno)  
   ...
   If Malc_HHasKey(hResponse, [message])
   ...
   If Malc_HHasKey(hResponse, [status]) .and. hResponse["status"] == [ERROR]
   ...
   If Malc_HHasKey(hResponse, [atividade_principal])
   ...
   If Malc_HHasKey(hResponse, [atividades_secundarias])
   ...
   If Malc_HHasKey(hResponse, [qsa]) .and. ValType(hResponse["qsa"]) == "A"
   ...
   If Malc_HHasKey(hResponse, [simples]) .and. ValType(hResponse["simples"]) == [H]
      ::lCnpj_OptanteSimples:= hb_defaultValue(hResponse["simples"]["optante"], .F.)
   Endif

   ::lCnpj_OptanteSimei:= .F.
   If Malc_HHasKey(hResponse, [simei]) .and. ValType(hResponse["simei"]) == [H]
...
Static Function Malc_At( cSearch, cText, nStart )
   Local nPos

   hb_Default( @nStart, 1 )
   IF nStart <= 1
      Return At( cSearch, cText )
   ENDIF

   nPos := At( cSearch, SubStr( cText, nStart ) )
Return (IIf( nPos == 0, 0, nPos + nStart - 1 ))

Static Function Malc_GetCurrentFolder()
   Local cDir := CurDir()
Return (IIf( Empty( cDir ), ".", cDir ))

Static Function Malc_Hash()
Return (hb_Hash())

Static Function Malc_HHasKey( hHash, xKey )
Return (ValType( hHash ) == "H" .AND. hb_HHasKey( hHash, xKey ))
